    ...
    try {
        ...
        childProcess.on('close', (code) => {
            if (code === 0) {
                ...
                console.error(`❌ Icon generation hatası (kategori: ${category}, exit code: ${code})`);
            }
        });
...
router.get('/gallery', async (req, res) => {
    try {
            ...
            if (!supabaseError && supabaseFiles) {
                supabaseFiles.forEach(file => {
                    if (file.name.endsWith('.jpg') || file.name.endsWith('.png')) {
                        ...
                        const nameParts = file.name.replace(/\.(jpg|png)$/, '').split('_');
                        const category = nameParts[0] || 'Unknown';
                        const accessoryName = nameParts.slice(1).join(' ').replace(/_/g, ' ') || 'Unknown';
        ...
        if (fs.existsSync(GENERATED_ICONS_DIR)) {
            const localFiles = fs.readdirSync(GENERATED_ICONS_DIR)
                ...
                .map(file => {
                    ...
                    // Dosya adından kategori ve accessory adını çıkar
                    const nameParts = file.replace(/\.(jpg|png)$/, '').split('_');
                    const category = nameParts[0] || 'Unknown';
                    const accessoryName = nameParts.slice(1).join(' ').replace(/_/g, ' ') || 'Unknown';
            ...
            localFiles.forEach(localFile => {
                const exists = images.find(img => img.filename === localFile.filename);
                if (!exists) {
    ...
    try {
        ...
        try {
            ...
            if (!error && supabaseFiles) {
                supabaseIconCount = supabaseFiles.filter(f => 
        ...
        if (fs.existsSync(GENERATED_ICONS_DIR)) {
            localIconCount = fs.readdirSync(GENERATED_ICONS_DIR)
                .filter(f => f.endsWith('.jpg') || f.endsWith('.png')).length;
        }
        
        const reportExists = fs.existsSync(path.join(GENERATED_ICONS_DIR, 'generation-report.json'));
        ...
        res.json({
            ...
            status: {
                ...
                totalAccessories: Object.values(accessoryLibrary).reduce((sum, items) => sum + items.length, 0),
