    ...
    if (!pendingGeneration) {
      ...
      if (creditDeducted && userId && userId !== "anonymous_user") {
        try {
          ...
          console.log("💰 Kredi iade edildi (pending generation hatası)");
        } catch (refundError) {
...
router.get("/change-color/user-generations/:userId", async (req, res) => {
  try {
    ...
    console.log(
      `📊 [CHANGE_COLOR] ${userId} için user generations sorgulanıyor... Status filter: ${
        status || "all"
      }`
    );
    ...
    console.log(
      `✅ ${generations?.length || 0} user generation bulundu (status: ${
        status || "all"
      })`
    );
