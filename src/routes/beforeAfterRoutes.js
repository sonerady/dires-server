...
async function enhancePromptWithGemini(
  ...
  isProblemSolutionMode = false, // Problem/Solution modu mu?
  problemText = null, // Kullanıcının girdiği problem açıklaması (opsiyonel)
  isBeforeAfterMode = false, // Before/After modu mu?
  beforeAfterContext = null // Kullanıcının girdiği before/after bağlam açıklaması (opsiyonel)
) {
  try {
    logger.log(
    ...
    logger.log("📊 [GEMINI] Infographic mode:", isInfographicMode);
    logger.log("🌿 [GEMINI] Lifestyle mode:", isLifestyleMode);
...
`;
    } else if (isProblemSolutionMode) {
      // PROBLEM/SOLUTION MODE - Before/After problem-solution comparison image
      promptForGemini = `
You are a world-class e-commerce visual strategist and conversion-focused product photographer specializing in Amazon, Etsy, Shopify, and D2C brand imagery. Your task is to analyze the product image provided and generate a SINGLE comprehensive image generation prompt that will create a powerful PROBLEM vs. SOLUTION comparison image — the #1 highest-converting image type in e-commerce.

⚠️ ABSOLUTE CRITICAL RULE - DO NOT CHANGE THE PRODUCT:
- The product in the generated image MUST be EXACTLY the same as the reference image
- DO NOT change the product's color, shape, design, brand, or any visual aspect
- The product must appear IDENTICAL to the original — same color, same material, same everything
- You are ONLY creating a PROBLEM/SOLUTION context around the UNCHANGED product
...
The image MUST be a SPLIT COMPOSITION showing two clear sides:

❌ LEFT SIDE — THE PROBLEM (Before / Without the product):
- Show the FRUSTRATING, MESSY, or DIFFICULT situation that exists WITHOUT this product
- Use slightly desaturated, cooler tones to convey negativity/frustration
- Include visual cues of chaos, mess, struggle, or inconvenience
- A person can be shown looking frustrated, struggling, or dealing with the problem
- The scene should immediately make the viewer think "I hate when that happens"
- Include a prominent ❌ or "BEFORE" visual indicator

✅ RIGHT SIDE — THE SOLUTION (After / With the product):
- Show the CLEAN, EASY, SATISFYING result when using this product
- Use warm, bright, vibrant tones to convey satisfaction and relief
- The product should be prominently visible and clearly being used
- A person can be shown looking satisfied, relaxed, or enjoying the result
- The scene should make the viewer think "I need this in my life"
- Include a prominent ✅ or "AFTER" visual indicator

${problemText ? `
=== USER-SPECIFIED PROBLEM (MUST FOLLOW) ===
The user has described the specific problem: "${problemText}"
...
🔴 SPLIT LAYOUT:
- Clear vertical split — LEFT side is the problem, RIGHT side is the solution
- Both sides should be in the SAME environment/context for direct comparison
- A clear visual divider (line, gradient, or contrast shift) between the two sides
- The split should feel like a magazine ad or premium Amazon A+ content

🔴 TEXT OVERLAYS (CRITICAL FOR E-COMMERCE):
- Include bold, clean text callouts:
  - Left side: "WITHOUT [Product]" or "BEFORE" or "The Problem" with ❌ icon
...
- Optional: Add small bullet points highlighting 2-3 key pain points vs. benefits

🔴 LIGHTING & COLOR:
- LEFT (Problem): Slightly darker, cooler color temperature, desaturated
- RIGHT (Solution): Brighter, warmer color temperature, vibrant and inviting
- The contrast should be OBVIOUS but not cartoonish — premium e-commerce quality
- Studio-quality lighting throughout — no amateur look

🔴 EMOTIONAL IMPACT:
- The viewer should FEEL the frustration on the left and RELIEF on the right
- Use body language: frustrated gestures (left) vs. relaxed/happy gestures (right)
- The transition from problem to solution should feel satisfying and aspirational

=== PRODUCT CATEGORY PROBLEM EXAMPLES (USE AS REFERENCE) ===

🏠 Home/Kitchen:
- ❌ Messy, cluttered counter → ✅ Organized, clean space with product
- ❌ Burnt/ruined food → ✅ Perfectly cooked with product
- ❌ Stained/dirty surface → ✅ Sparkling clean with product

👕 Fashion/Accessories:
- ❌ Wrinkled/messy outfit → ✅ Crisp, polished look with product
- ❌ Items falling/disorganized → ✅ Neatly arranged with product
- ❌ Uncomfortable posture → ✅ Comfortable, confident posture with product

📱 Electronics/Tech:
- ❌ Tangled cables, dead battery → ✅ Clean setup, fully charged with product
- ❌ Phone falling/cracking → ✅ Phone secure and protected with product
- ❌ Poor audio/video quality → ✅ Crystal clear with product

💪 Fitness/Health:
- ❌ Pain, discomfort, bad form → ✅ Comfort, proper support with product
- ❌ Messy workout space → ✅ Organized, motivating setup with product

🔴 WHAT MAKES PROBLEM/SOLUTION IMAGES SELL:
- They create INSTANT emotional connection — the viewer sees their own frustration
- The product becomes the HERO that saves them from the problem
- Before/after format is the most shared and engaged-with content on social media
- Amazon data shows problem/solution images increase click-through rates by 20-35%
- They answer the #1 buyer question: "Why do I need this?"

=== OUTPUT ===
Generate ONLY the final image generation prompt. Do NOT include these instructions or commentary. The prompt MUST create a professional split-composition problem/solution image with clear BEFORE (problem without product) and AFTER (solution with product) sides. Include text overlays with ❌/✅ indicators. Use contrasting color tones (cool/dark for problem, warm/bright for solution). The product's appearance must match the reference image EXACTLY.

${originalPrompt ? `Additional context from user: ${originalPrompt}` : ""}
`;
    } else if (isBeforeAfterMode) {
      // BEFORE/AFTER MODE - Transformation before/after product photography
      promptForGemini = `
You are a world-class e-commerce product photographer and visual storyteller specializing in before/after transformation imagery for Amazon, Etsy, Shopify, and premium D2C brands. Your expertise is in cleaning products, cosmetics, skincare, home care, restoration products, and any product that creates a visible TRANSFORMATION. Your task is to analyze the product image provided and generate a SINGLE comprehensive image generation prompt that will create a stunning, high-converting before/after transformation photo.

⚠️ ABSOLUTE CRITICAL RULE - THE PRODUCT MUST BE VISIBLE:
- The ACTUAL PRODUCT from the reference image MUST appear prominently in the generated image
- The product must be EXACTLY as shown in the reference — same packaging, same colors, same labels, same brand
- DO NOT change the product's appearance in any way
- The product should be the HERO of the composition — the star that connects the before and after

⚠️ CRITICAL: THIS IS A TRANSFORMATION IMAGE, NOT A PROBLEM/SOLUTION IMAGE:
- The focus is on showing a DRAMATIC VISUAL TRANSFORMATION — a dirty thing becoming clean, a dull surface becoming shiny, aged skin becoming youthful
- The product MUST be clearly visible as the agent of change — positioned between, alongside, or prominently in the "after" side
- The before and after should show the SAME OBJECT/SURFACE/AREA in two states

IMPORTANT: Analyze the product image carefully to understand:
1. What type of product it is (cleaner, cosmetic, paint, polish, cream, serum, etc.)
2. What TRANSFORMATION this product creates (cleaning, beautifying, restoring, protecting, etc.)
3. What the realistic "before" state looks like (dirty, stained, dull, aged, damaged, rough, etc.)
4. What the stunning "after" state looks like (clean, sparkling, smooth, radiant, restored, etc.)

${beforeAfterContext ? `
=== USER-SPECIFIED TRANSFORMATION CONTEXT (MUST FOLLOW) ===
The user has described the transformation: "${beforeAfterContext}"
You MUST use this specific before/after scenario. Show the exact transformation described.
` : `
=== AUTO-DETECT BEST TRANSFORMATION ===
Since the user didn't specify, analyze the product and determine:
- What surface/object this product is typically used on
- What the most DRAMATIC and VISUALLY SATISFYING transformation would be
- Choose a transformation that creates maximum "wow factor" for e-commerce
`}

=== BEFORE/AFTER TRANSFORMATION CATEGORIES (USE AS REFERENCE) ===

🧹 Cleaning Products (Detergents, Sprays, Wipes):
- BEFORE: Heavily stained pan with burnt food residue, grease spots, blackened bottom
- AFTER: Mirror-shine pan, reflective surface, gleaming stainless steel, brand new appearance
- PRODUCT PLACEMENT: Bottle/spray standing next to or behind the sparkling clean pan

🧴 Skincare & Cosmetics:
- BEFORE: Dull, dry, textured skin with visible pores, fine lines, uneven tone
- AFTER: Dewy, glowing, smooth skin with healthy radiance, even complexion
- PRODUCT PLACEMENT: Serum/cream tube artfully positioned near the transformed skin

🏠 Home Care (Floor cleaners, Wood polish, etc.):
- BEFORE: Scuffed, dusty, water-stained floor/surface with visible dirt and grime
- AFTER: Gleaming, reflective, restored surface that looks brand new
- PRODUCT PLACEMENT: Product bottle standing on the clean section

👟 Shoe/Leather Care:
- BEFORE: Scuffed, dirty, faded shoes/leather with visible wear marks
- AFTER: Polished, conditioned, vibrant leather with deep color restored
- PRODUCT PLACEMENT: Care product jar/tube next to the restored item

🚗 Car Care:
- BEFORE: Swirled, oxidized, dull paint/wheels with water spots and contamination
- AFTER: Deep gloss, mirror-like reflection, showroom-quality finish
- PRODUCT PLACEMENT: Wax/polish bottle beside the gleaming surface

🪥 Dental/Personal Care:
- BEFORE: Yellow, stained, dull appearance
- AFTER: Bright, white, clean, fresh appearance
- PRODUCT PLACEMENT: Product clearly visible as the transformation agent

=== VISUAL COMPOSITION REQUIREMENTS (CRITICAL) ===

🔴 LAYOUT — SPLIT OR DIAGONAL COMPOSITION:
- Clear visual split showing BEFORE on one side and AFTER on the other
- Options: vertical split (left/right), diagonal split, or top/bottom split
- The transition line should be DRAMATIC — showing the exact moment of transformation
- Think of it like a magic wand sweeping across: dirty on one side, spotless on the other

🔴 THE PRODUCT AS THE HERO (MOST IMPORTANT):
- The product MUST be prominently visible in the composition
- Best placement options:
  1. CENTER: Product standing at the dividing line between before and after
  2. AFTER SIDE: Product placed on/near the transformed surface, taking credit for the result
  3. FOREGROUND: Product in the foreground with the transformation happening behind it
- The product should be well-lit, sharp, and its label/branding clearly readable
- The product should feel like it BELONGS in the scene — naturally integrated, not pasted on

🔴 THE TRANSFORMATION MUST BE DRAMATIC:
- The contrast between before and after should be EXTREME and SATISFYING
- BEFORE: Genuinely dirty, stained, damaged, dull — make viewers cringe slightly
- AFTER: Impossibly clean, sparkling, restored — make viewers say "wow!"
- The same surface/object/area must be recognizable in both states
- Include realistic details: water droplets on clean surfaces, shine reflections, smooth textures

🔴 LIGHTING & ATMOSPHERE:
- BEFORE side: Slightly flat, unflattering light that emphasizes imperfections
- AFTER side: Beautiful, aspirational lighting with highlights and specular reflections
- The product itself should ALWAYS be beautifully lit regardless of which side it's on
- Studio-quality photography throughout — premium e-commerce standard
- Consider rim lighting on the product to make it pop from the background

🔴 TEXT OVERLAYS (FOR E-COMMERCE IMPACT):
- "BEFORE" label on the untreated side
- "AFTER" label on the transformed side
- Optional: Arrow or sweeping motion graphic showing the direction of transformation
- Clean, modern typography — white text with subtle shadow for readability
- The product name/brand can be subtly highlighted

🔴 REALISM & CREDIBILITY:
- The transformation must look BELIEVABLE — not photoshopped or fake
- Real-world context: kitchen counter, bathroom tile, car hood, skin close-up
- Include environmental details that ground the scene (countertop texture, bathroom tiles, etc.)
- The "after" should look achievable — aspirational but not impossible
- Slight imperfections in the "after" can actually increase credibility

=== COLOR PSYCHOLOGY (CRITICAL FOR CONVERSION) ===
- BEFORE: Yellowish, brownish, grey tones — colors associated with dirt, neglect, age
- AFTER: Cool whites, bright blues, warm golds — colors associated with cleanliness, newness, luxury
- TRANSITION: A satisfying gradient or sharp line where dirty meets clean
- PRODUCT: Vibrant, true-to-reference colors — the product should "pop" against both backgrounds

=== WHAT MAKES BEFORE/AFTER IMAGES SELL ===
- They provide VISUAL PROOF that the product works — the #1 conversion factor
- Customers can IMAGINE their own dirty pan/stained shirt/dull skin being transformed
- Before/after is the most SHARED content type for cleaning/beauty products on social media
- Amazon data shows before/after images increase conversion by 25-40% for cleaning products
- The product's physical presence in the image builds trust: "this is what does the magic"

=== OUTPUT ===
Generate ONLY the final image generation prompt. Do NOT include these instructions or commentary. The prompt MUST create a professional before/after transformation image where:
1. The PRODUCT from the reference image is clearly visible and prominently placed
2. A dramatic BEFORE state (dirty/damaged/dull) transitions to an AFTER state (clean/restored/radiant)
3. The same surface/object is shown in both states for direct comparison
4. Beautiful studio-quality lighting emphasizes the transformation
5. The product's appearance matches the reference image EXACTLY
Include "BEFORE" and "AFTER" text labels. Make the transformation dramatic and satisfying.
...
`;
      ...
      // INFOGRAPHIC MODE - Amazon/Etsy tarzı ürün infografik oluşturma
      promptForGemini = `
You are a professional Amazon/Etsy product infographic designer. Your task is to analyze the product image provided and generate a SINGLE comprehensive image generation prompt that will create a professional product infographic.

⚠️ ABSOLUTE CRITICAL RULE - DO NOT CHANGE THE PRODUCT:
- The product in the generated infographic MUST be EXACTLY the same as the reference image
- DO NOT change the product's color, shape, design, brand, or any visual aspect
- DO NOT invent a new colorway, DO NOT modify textures, DO NOT alter any part of the product
- The product must appear IDENTICAL to the original reference photo - same color, same design, same everything
- You are ONLY adding infographic elements (callouts, text, icons, lines) AROUND the unchanged product
- Think of it as overlaying infographic annotations on top of the original product photo

IMPORTANT: Analyze the product image carefully to understand:
1. What type of product it is (clothing, electronics, accessories, home goods, etc.)
2. Key features and materials visible IN THE ACTUAL IMAGE
3. Design details and unique selling points OF THE ACTUAL PRODUCT AS SHOWN
4. The EXACT colors, patterns, and appearance of the product - describe them accurately

Then generate a prompt for creating a PROFESSIONAL PRODUCT INFOGRAPHIC IMAGE:

=== CRITICAL: AMATEUR-TO-PROFESSIONAL TRANSFORMATION ===

The input image is likely an AMATEUR photo taken with a phone or basic camera. Your generated prompt MUST include instructions to fix ALL of the following amateur photography problems:

🔴 LIGHTING & COLOR CORRECTION (CRITICAL):
- FIX harsh shadows, uneven lighting, yellow/warm color casts from indoor lighting
- CORRECT white balance — ensure true-to-life, accurate color reproduction
- REPLACE amateur flat/dull lighting with professional, even studio lighting that highlights product form and details
- REMOVE overexposed highlights and underexposed shadow areas
- Ensure colors are vibrant, accurate, and consistent — the product should look exactly as it would under professional studio lights

🔴 PRODUCT CLEANUP & RESTORATION (CRITICAL):
- REMOVE all visible imperfections: dust particles, lint, loose threads, fingerprints, smudges, scratches, stains, price tags, stickers, barcodes
- ELIMINATE wrinkles, creases, and folds — present the product as freshly pressed, pristine, and brand-new
- FIX any sagging, deformation, or poor product positioning — the product must look structured, firm, and premium
- REMOVE background clutter, distracting elements, messy surfaces, other objects that are not the product
- CLEAN UP any reflections, glare, or lens flare from amateur photography

🔴 SHARPNESS & DETAIL ENHANCEMENT (CRITICAL):
- FIX any blur, softness, or out-of-focus areas — the product must be razor-sharp with maximum detail clarity
- ENHANCE material textures: show leather grain, fabric weave, metal polish, plastic sheen, wood grain etc. at their best
- CORRECT any motion blur or camera shake artifacts
- Ensure stitching, seams, logos, labels, hardware, zippers, buttons, and all construction details are crisp and visible

🔴 PERSPECTIVE & DISTORTION FIX (CRITICAL):
- CORRECT any wide-angle lens distortion from phone cameras (barrel distortion, stretched edges)
- FIX tilted/crooked product positioning — the product must appear perfectly straight and well-composed
- CORRECT any keystoning or perspective distortion that makes the product look skewed
- Ensure proportions are accurate and true to real life — no stretched or squished appearance

🔴 BACKGROUND & ISOLATION (CRITICAL):
- COMPLETELY REMOVE the original amateur background (messy desk, bed, floor, hand holding product, etc.)
- REPLACE with a clean, pure white or very light professional background
- The product must appear as if photographed in a professional studio with proper isolation
- NO traces of the original environment should remain

🔴 OVERALL QUALITY STANDARD (CRITICAL):
- The final result must be INDISTINGUISHABLE from a photo taken by a professional product photographer in a fully equipped studio
- Think: the quality level you see on Apple product pages, premium Amazon A+ listings, or luxury brand websites
- The product must look brand-new, pristine, and premium — as if straight from the factory or a luxury boutique display
- Every detail must convey quality, professionalism, and trustworthiness to potential buyers

=== INFOGRAPHIC DESIGN REQUIREMENTS ===

PRODUCT PRESERVATION (MOST IMPORTANT):
- The product MUST remain EXACTLY as shown in the reference image - same colors, same design, same appearance
- Describe the product's ACTUAL colors and features as they appear in the reference photo
- DO NOT suggest any color changes, modifications, or "new colorways"
- While you MUST fix amateur photo quality issues (lighting, sharpness, dust, background), the product itself (color, design, shape, brand) must stay IDENTICAL

LAYOUT:
- Clean, modern, professional infographic layout suitable for Amazon/Etsy product listings
- The ORIGINAL product image should be prominently featured in the center as the main visual element
- Surrounding the product, add 4-6 callout sections highlighting key features
- Use clean lines, arrows, or connector lines pointing from callout boxes to specific product areas
- White or very light background for maximum readability

CALLOUT SECTIONS (analyze the ACTUAL product and include relevant ones):
- Material & Quality (based on what you SEE in the actual product image)
- Key Features (based on visible features in the actual product)
...
- Use a consistent color scheme that complements the product's ACTUAL colors
- Modern sans-serif typography for text elements
- Icons or small illustrations next to each callout point
- High contrast for readability

TEXT ELEMENTS:
- Product title/name at the top in bold
...
- Etsy listing infographic style
- Professional e-commerce product presentation

PERSPECTIVE, FRAMING & CAMERA ANGLE (CRITICAL):
- You MUST determine the BEST perspective and camera angle for this specific product type based on how it is most commonly and effectively displayed in real e-commerce listings
- Analyze the product category and choose the perspective that BEST showcases its features:
  • Shoes/Sneakers → 3/4 angle from slightly above, showing both the side profile and top details
  • Clothing/T-shirts → Front-facing flat lay or ghost mannequin view
  • Watches/Jewelry → Slight top-down angle with tilt to show face and band/chain
  • Bags/Backpacks → 3/4 front angle showing depth and structure
  • Electronics/Gadgets → Hero angle (slightly below eye level) for premium feel
  • Bottles/Skincare → Straight-on front view or slight 3/4 with label visible
  • Furniture → 3/4 perspective showing depth and proportions
  • Food products → Top-down or 45-degree angle for appetizing presentation
  • Tools/Equipment → Slight angle showing the grip/handle and working end
- The product should be photographed from the angle that a professional Amazon seller would use for their MAIN listing image
- Ensure the framing is tight enough to show details but wide enough for callout annotations
- The product must fill approximately 60-70% of the image area, leaving space for infographic elements
- Use the perspective that makes the product look most appealing, professional, and true to its real-world usage

${productFeatures ? `
=== USER-PROVIDED PRODUCT FEATURES (MUST USE THESE) ===
The user has provided the following product features/specifications. You MUST use these as the callout sections in the infographic instead of guessing. Include ALL of these features as callout points:
...
The output must be hyper-realistic, high-end professional fashion editorial quality, suitable for commercial catalog presentation.
    ...
    try {
      ...
      if (isRefinerMode) {
        ...
        // Before/After mode için fallback prompt
        logger.log(
          "🔄 [CATCH-BEFORE-AFTER] Gemini başarısız, before/after fallback prompt kullanılıyor"
        );
        enhancedPrompt = `Create a professional before/after transformation product image. The PRODUCT from the reference must be prominently visible in the composition. Show a dramatic split composition: BEFORE side shows a dirty, stained, dull, or damaged surface/object. AFTER side shows the same surface/object transformed to sparkling clean, restored, or radiant state. ${beforeAfterContext ? `Transformation: ${beforeAfterContext}. ` : "Show the most dramatic transformation this product creates. "}The product must be clearly visible as the agent of change. Include "BEFORE" and "AFTER" text labels. Use contrasting lighting: flat unflattering light on before side, beautiful aspirational lighting on after side. The product must remain exactly as shown in the reference. Professional e-commerce quality suitable for Amazon/Etsy listings.`;
      } else if (isProblemSolutionMode) {
        // Problem/Solution mode için fallback prompt
        logger.log(
          "❌✅ [CATCH-PROBLEM] Gemini başarısız, problem-solution fallback prompt kullanılıyor"
        );
        enhancedPrompt = `Create a professional split-composition product image showing PROBLEM vs SOLUTION. Left side (❌ BEFORE/WITHOUT): Show a frustrating, messy, or difficult scenario without the product - desaturated, cooler tones, visual chaos. ${problemText ? `Problem: ${problemText}. ` : "Show the most relatable problem this product solves. "}Right side (✅ AFTER/WITH): Show the clean, easy, satisfying result with the product prominently visible and being used - warm, bright, vibrant tones. Clear vertical split divider between the two sides. Include bold text overlays: "WITHOUT" with ❌ on left, "WITH" with ✅ on right. The product must remain exactly as shown in the reference - same colors, design, and appearance. Professional e-commerce quality suitable for Amazon/Etsy A+ content.`;
      } else if (isDetailCloseupMode) {
...
Model, garment, and environment must integrate into one cohesive, seamless professional photo suitable for commercial catalogs and editorial campaigns.`;
    ...
    if (enhancedPrompt === originalPrompt) {
      ...
      // ❌✅ PROBLEM/SOLUTION MODE için özel fallback prompt
      // 🔄 BEFORE/AFTER MODE için özel fallback prompt
      if (isBeforeAfterMode) {
        logger.log(
          "🔄 [FALLBACK-BEFORE-AFTER] Before/After mode fallback prompt kullanılıyor"
        );
        return `Create a professional before/after transformation product image. The PRODUCT from the reference must be visible. Split composition: BEFORE (dirty/damaged/dull) to AFTER (clean/restored/radiant). ${beforeAfterContext ? `Context: ${beforeAfterContext}. ` : "Auto-detect best transformation. "}Include text labels. Professional e-commerce quality.`;
      }

      if (isProblemSolutionMode) {
        logger.log(
          "❌✅ [FALLBACK-PROBLEM] Problem/Solution mode fallback prompt kullanılıyor"
        );
        return `Create a professional split-composition product image showing PROBLEM vs SOLUTION. Left side (❌ BEFORE/WITHOUT): Show a frustrating scenario without the product - desaturated, cooler tones. ${problemText ? `Problem: ${problemText}. ` : "Show the most relatable problem this product solves. "}Right side (✅ AFTER/WITH): Show the satisfying result with the product - warm, bright tones. Include bold text overlays with ❌/✅ indicators. The product must match the reference exactly. Professional e-commerce quality.`;
      }
...
router.post("/generate", async (req, res) => {
  ...
  try {
    let {
      ...
      // Problem/Solution mode specific parameters
      isProblemSolutionMode = false, // Problem/Solution modu mu?
      problemText = null, // Kullanıcının girdiği problem açıklaması (opsiyonel)
      // Before/After mode specific parameters
      isBeforeAfterMode = false, // Before/After modu mu?
      beforeAfterContext = null, // Kullanıcının girdiği before/after bağlam açıklaması (opsiyonel)
    ...
    const settingsWithSession = {
      ...
      ...(isBeforeAfterMode && { isBeforeAfterMode: true }),
    };
    ...
    if (isBeforeAfterMode || isProblemSolutionMode || isDetailCloseupMode || isLifestyleMode || isInfographicMode || isColorChange || isPoseChange || isRefinerMode) {
      // 🔄 BEFORE/AFTER MODE, ❌✅ PROBLEM/SOLUTION MODE, 🔍 DETAIL CLOSEUP MODE, 🌿 LIFESTYLE MODE, 📊 INFOGRAPHIC MODE, 🎨 COLOR CHANGE MODE, 🕺 POSE CHANGE MODE veya 🔧 REFINER MODE
      if (isBeforeAfterMode) {
        logger.log(
          "🔄 Before/After mode: Gemini ile before/after transformation görseli prompt'u oluşturuluyor"
        );

        enhancedPrompt = await enhancePromptWithGemini(
          promptText ||
            "Create a professional before/after transformation product image showing dramatic results",
          finalImage,
          settings || {},
          locationImage,
          poseImage,
          ...
          true, // addTextOverlay - before/after always has text
          false, // isDetailCloseupMode
          null, // detailFocus
          "single", // detailLayout
          "soft", // detailLightingStyle
          "bokeh", // detailBackgroundStyle
          false, // isProblemSolutionMode
          null, // problemText
          true, // isBeforeAfterMode
          beforeAfterContext // before/after context from user
        );
      } else if (isProblemSolutionMode) {
        logger.log(
          "❌✅ Problem/Solution mode: Gemini ile problem-solution karşılaştırma görseli prompt'u oluşturuluyor"
        );

        enhancedPrompt = await enhancePromptWithGemini(
          promptText ||
            "Create a professional problem vs solution comparison product image showing before/after with the product",
          finalImage,
          settings || {},
          locationImage,
          poseImage,
          ...
          null, // lifestyleScene
          "single", // lifestyleLayout
          true, // addTextOverlay - problem/solution always has text
          false, // isDetailCloseupMode
          null, // detailFocus
          "single", // detailLayout
          "soft", // detailLightingStyle
          "bokeh", // detailBackgroundStyle
          true, // isProblemSolutionMode
          problemText // problem text from user
        );
      } else if (isDetailCloseupMode) {
        logger.log(
        ...
        enhancedPrompt = await enhancePromptWithGemini(
          promptText ||
            "Create a professional lifestyle product photo with natural setting and beautiful composition",
          finalImage,
          settings || {},
          locationImage,
          poseImage,
        ...
        logger.log(
          "📊 Infographic mode: Gemini ile Amazon/Etsy tarzı infografik prompt'u oluşturuluyor"
        );

        // Infographic mode için Gemini ile gelişmiş prompt oluştur
        enhancedPrompt = await enhancePromptWithGemini(
          promptText ||
            "Create a professional Amazon/Etsy style product infographic with key features, benefits, and specifications highlighted in a clean, modern layout",
          finalImage,
          settings || {},
          locationImage,
          poseImage,
    ...
    // 🌿 LIFESTYLE MODE & 📊 INFOGRAPHIC MODE: nano-banana-2 kullanır (normal flow'a düşer)
    // Bu modlar için ayrı GPT Image 1.5 bloğu yok - normal nano-banana-2 akışına devam eder
