  ...
  try {
    ...
    try {
      const result = await supabase
        .from("users")
        .select(
          "credit_balance, is_pro, subscription_type, received_initial_credit, owner, team_max_members, team_subscription_active"
        )
        .eq("id", userId)
        .single();
      ...
      // Bazı columnlar yoksa, owner ile dene
      try {
        const result = await supabase
          .from("users")
        ...
        // owner column yoksa, sadece temel alanları al
        const result = await supabase
          .from("users")
          .select("credit_balance, is_pro")
          .eq("id", userId)
