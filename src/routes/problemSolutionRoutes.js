...
async function enhancePromptWithGemini(
  ...
  contrastLevel = "dramatic", // "subtle" = hafif, "dramatic" = güçlü
...
🔴 SPLIT LAYOUT — ${splitDirection === "horizontal" ? "HORIZONTAL (TOP/BOTTOM)" : "VERTICAL (LEFT/RIGHT)"}:
${splitDirection === "horizontal" ? `- Clear HORIZONTAL split — TOP half is the problem, BOTTOM half is the solution
- The top section shows the frustrating scenario, the bottom section shows the satisfying solution
- A clear horizontal divider (line, gradient, or contrast shift) between the two halves` : `- Clear VERTICAL split — LEFT side is the problem, RIGHT side is the solution
- A clear vertical divider (line, gradient, or contrast shift) between the two sides`}
...
🔴 LIGHTING & COLOR — ${contrastLevel === "subtle" ? "SUBTLE CONTRAST" : "DRAMATIC CONTRAST"}:
${contrastLevel === "subtle" ? `- LEFT/TOP (Problem): Slightly cooler tones, marginally desaturated — the difference should be noticeable but elegant
- RIGHT/BOTTOM (Solution): Slightly warmer tones, marginally more vibrant
- The contrast should be REFINED and PREMIUM — more like a high-end catalog, less like a social media before/after
- Both sides should look professional and polished` : `- LEFT/TOP (Problem): Noticeably darker, cooler color temperature, clearly desaturated
- RIGHT/BOTTOM (Solution): Bright, warm color temperature, vibrant and inviting
- The contrast should be OBVIOUS and IMPACTFUL but not cartoonish — premium e-commerce quality
- The color shift should immediately communicate "bad → good"`}
- Studio-quality lighting throughout — no amateur look

🔴 SCENE ENVIRONMENT — ${sceneType === "studio" ? "STUDIO" : "LIFESTYLE/REALISTIC"}:
${sceneType === "studio" ? `- Use a CLEAN STUDIO environment for both sides
- White/light gray seamless background with professional studio lighting
- The product and scenario should be clearly staged in a controlled studio setting
- Focus is on the product and the problem/solution — no distracting environmental elements
- Think: premium Amazon main image quality but in split comparison format` : `- Use a REALISTIC LIFESTYLE environment for both sides
- Show the product in its NATURAL context of use (kitchen, office, gym, bedroom, etc.)
- The environment should feel authentic, relatable, and aspirational
- Include contextual elements that reinforce the problem/solution narrative
- Think: Amazon lifestyle gallery image — authentic but professional`}

🔴 EMOTIONAL IMPACT:
- The viewer should FEEL the frustration on the ${splitDirection === "horizontal" ? "top" : "left"} and RELIEF on the ${splitDirection === "horizontal" ? "bottom" : "right"}
- Use body language: frustrated gestures (problem) vs. relaxed/happy gestures (solution)
...
The output must be hyper-realistic, high-end professional fashion editorial quality, suitable for commercial catalog presentation.
    ...
    try {
      ...
      if (isRefinerMode) {
        ...
        enhancedPrompt = `Create a professional ${splitDirection === "horizontal" ? "horizontal (top/bottom)" : "vertical (left/right)"} split-composition product image showing PROBLEM vs SOLUTION. ${splitDirection === "horizontal" ? "Top half" : "Left side"} (❌ BEFORE/WITHOUT): Show a frustrating, messy, or difficult scenario without the product - ${contrastLevel === "subtle" ? "slightly cooler tones, marginally desaturated" : "desaturated, cooler tones, visual chaos"}. ${problemText ? `Problem: ${problemText}. ` : "Show the most relatable problem this product solves. "}${splitDirection === "horizontal" ? "Bottom half" : "Right side"} (✅ AFTER/WITH): Show the clean, easy, satisfying result with the product prominently visible and being used - ${contrastLevel === "subtle" ? "slightly warmer, refined tones" : "warm, bright, vibrant tones"}. Clear ${splitDirection === "horizontal" ? "horizontal" : "vertical"} split divider between the two sides. ${addTextOverlay ? `Include bold text overlays: "WITHOUT" with ❌ on ${splitDirection === "horizontal" ? "top" : "left"}, "WITH" with ✅ on ${splitDirection === "horizontal" ? "bottom" : "right"}.` : "Do NOT include any text or labels on the image."} ${sceneType === "studio" ? "Use a clean studio environment." : "Use a realistic lifestyle environment."} The product must remain exactly as shown in the reference - same colors, design, and appearance. Professional e-commerce quality suitable for Amazon/Etsy A+ content.`;
      } else if (isDetailCloseupMode) {
...
router.post("/generate", async (req, res) => {
  ...
  try {
    ...
    const settingsWithSession = {
      ...
      ...(isProblemSolutionMode && { isProblemSolutionMode: true }),
    };
    ...
    if (isProblemSolutionMode || isDetailCloseupMode || isLifestyleMode || isInfographicMode || isColorChange || isPoseChange || isRefinerMode) {
      // ❌✅ PROBLEM/SOLUTION MODE, 🔍 DETAIL CLOSEUP MODE, 🌿 LIFESTYLE MODE, 📊 INFOGRAPHIC MODE, 🎨 COLOR CHANGE MODE, 🕺 POSE CHANGE MODE veya 🔧 REFINER MODE
      if (isProblemSolutionMode) {
        logger.log(
