...
router.get("/test", async (req, res) => {
  try {
    const womanHairStyles = await getImagesFromBucket(
      "woman_hair_style_images"
    );
    const manHairStyles = await getImagesFromBucket("man_hair_style_images");
...
router.get("/woman", async (req, res) => {
  try {
    ...
    const hairStyles = await getImagesFromBucket("woman_hair_style_images");
...
router.get("/man", async (req, res) => {
  try {
    ...
    const hairStyles = await getImagesFromBucket("man_hair_style_images");
    const { paginatedItems, hasMore, total } = paginateData(
      hairStyles,
      page,
      limit
