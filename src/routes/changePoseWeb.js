...
// @fal-ai/client import for GPT Image 2.5
const { fal } = require("@fal-ai/client");
fal.config({
  credentials: process.env.FAL_API_KEY,
});

// Fal.ai GPT Image 2.5 Edit API call using SDK (for Refiner mode - Ghost Mannequin style)
async function callFalAiGptImageEditForRefiner(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      // fal.queue.submit ile GPT Image 2.5'e istek gönder
...
async function ensureRemoteReferenceImage(imageEntry, userId) {
  ...
  if (typeof imageEntry === "string") {
    ...
    imageEntry = { uri: sanitizeImageUrl(imageEntry) };
  }

  const result = { ...imageEntry };
  const currentUri = result.uri || result.url || null;

  // Prompt analysis accepts remote URLs. Upload inline photos too, otherwise
  // the model photo is silently omitted and only the pose example is analyzed.
  if (currentUri?.startsWith("data:image/")) {
    result.uri = await uploadReferenceImageToSupabase(currentUri, userId);
    result.base64 = currentUri.slice(currentUri.indexOf(",") + 1);
...
// ============================================================
// Pose Change Generations tablosuna kayıt fonksiyonları
// reference_results'a ek olarak pose_change_generations tablosuna da yazar
// ============================================================

async function saveToPoseChangeGenerations({
  userId,
  generationId,
  status = "pending",
  originalPrompt = null,
  ...
  try {
    ...
    if (error) {
      console.error("❌ [POSE_TABLE] Kayıt hatası:", error.message);
      return null;
    }
    logger.log(`✅ [POSE_TABLE] Kayıt oluşturuldu: ${generationId} (${status})`);
    return data?.[0] || null;
  } catch (err) {
    console.error("❌ [POSE_TABLE] Exception:", err.message);
    return null;
  }
}

async function updatePoseChangeGeneration(generationId, userId, updates = {}) {
  try {
    const updateData = {
      ...updates,
    ...
    if (error) {
      console.error("❌ [POSE_TABLE] Güncelleme hatası:", error.message);
      return false;
    }
    logger.log(`✅ [POSE_TABLE] Güncellendi: ${generationId} →`, Object.keys(updates).join(", "));
    return true;
  } catch (err) {
    console.error("❌ [POSE_TABLE] Update exception:", err.message);
    return false;
  }
}
...
async function updateGenerationStatus(
  ...
  try {
    ...
    // 📊 Pose change tablosunu da güncelle (isPoseChange veya settings'den kontrol)
    const isPoseChangeGen = previousSettings?.isPoseChange || previousSettings?.pose || finalUpdates?.settings?.isPoseChange;
    if (isPoseChangeGen) {
      const poseUpdateData = { status };
      if (finalUpdates.result_image_url) {
        poseUpdateData.result_image_url = finalUpdates.result_image_url;
      }
      if (finalUpdates.enhanced_prompt) {
        poseUpdateData.enhanced_prompt = finalUpdates.enhanced_prompt;
      }
      if (finalUpdates.replicate_prediction_id) {
        poseUpdateData.fal_request_id = finalUpdates.replicate_prediction_id;
      }
      if (finalUpdates.processing_time_seconds) {
        poseUpdateData.processing_time_seconds = finalUpdates.processing_time_seconds;
      }
      await updatePoseChangeGeneration(generationId, userId, poseUpdateData);
    }
...
The output must be hyper-realistic, high-end professional fashion editorial quality, suitable for commercial catalog presentation.
    ...
    // 🕺 Pose change directive — V7 "Opening Directives" pattern.
    // Statik prepend yerine Gemini'ye intent + meta-instruction veriyoruz;
    // Gemini ⚠️ başlığı verbatim koruyup ALTI kıyafete + sahneye + kullanıcı
    // pozuna göre 2-3 cümle ENHANCED yazacak.
    if (isPoseChange) {
...
clothing and environment. Image 2, when supplied, is a pose example ONLY:
use its body positioning, never its identity, clothing or background.
Base all garment and scene descriptions on Image 1.
...
router.post("/generate", async (req, res) => {
  ...
  try {
    ...
    if (isPoseChange) {
      // referenceImageUrls sadece ürün fotoğraflarını içerir (model fotoğrafı line 3866'da splice edildi)
      const productImageUrls = referenceImageUrls || [];
      // Model fotoğrafı ayrıca ensureRemoteReferenceImage ile upload edildi (line 3891)
      const modelImageUrl = modelReferenceImage?.uri || null;

      logger.log(`📊 [POSE_TABLE] Kayıt verileri:`, {
        generationId: finalGenerationId,
        originalImageUrl: modelImageUrl?.substring(0, 80) || "YOK",
        poseImageUrl: poseImage?.substring?.(0, 80) || "YOK",
        productImages: productImageUrls.map(u => u?.substring(0, 80)),
        referenceImageUrlsCount: referenceImageUrls?.length || 0,
        gender: settings?.gender || "woman",
        customDetail: customDetail || "YOK",
      });

      await saveToPoseChangeGenerations({
        userId,
        generationId: finalGenerationId,
        status: "processing",
        originalPrompt: promptText,
        customDetail: customDetail || settings?.customDetail || null,
        originalImageUrl: modelImageUrl,
        poseImageUrl: poseImage || null,
      ...
      logger.log(`⚠️ [POSE_TABLE] isPoseChange=false, pose tablosuna kayıt atlanıyor`);
    }
    ...
    if (isColorChange || isPoseChange || isRefinerMode) {
      ...
      if (isColorChange) {
        ...
        enhancedPrompt = await enhancePromptWithGemini(
          ...
          userId, // Compress için userId
          modelReferenceImage?.base64 || null // Match the source model, not the product collage.
        );
      }
    ...
    // 🕺 Pose change directive artık V7 "Opening Directives" pattern üzerinden
    // upstream'de Gemini'ye verilip enhanced ediliyor (⚠️ başlık verbatim
    // korunuyor, alt 2-3 cümle kıyafete + sahneye göre özelleşiyor). Statik
    // post-Gemini prepend kaldırıldı.
    ...
    if (isRefinerMode) {
      ...
      try {
        ...
        logger.log(
          "✅ [REFINER MODE] GPT Image 2.5 başarılı:",
          gptImageResult
        );
