...
async function callReplicateNanoBananaPro(prompt, resultImageUrl, referenceImageUrl, maxRetries = 3, imageSize = "9:16") {
    ...
    console.log(`🎨 [STORY_FAL] kit route → GPT Image 2.5 (NB2 yedek), aspect: ${aspectRatio}`);
...
router.post("/generate-product-story", async (req, res) => {
    ...
    const STORY_GENERATION_COST = 30; // tüm kitler ortak 30 kredi
    ...
    try {
        ...
        if (recordId !== originalRecordId) {
            console.log(`🔄 [STORY] Using canonical record ID: ${recordId} (was: ${originalRecordId})`);
        }
            ...
            try {
                ...
                if (savedUrl && recordId) {
                    try {
                        ...
                        const result = await appendStoryToRecord(recordId, savedUrl, absIndex);
                        if (result !== null) {
