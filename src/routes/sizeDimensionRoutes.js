...
- ${sceneType === "studio" ? "Professional, clean studio look" : "Natural, realistic lifestyle environment that matches the product's use case"}
`}
...
${sceneType === "studio" ? `
🔴 BACKGROUND (STUDIO):
- CLEAN WHITE or very light neutral background — this is a SIZE REFERENCE image
- No distracting elements — pure focus on the product and its size context
- Professional studio setup — even, bright, no harsh shadows
...
- Place the product in a REALISTIC, natural environment that matches its use case
- Use a real-world setting: kitchen counter, desk, living room table, outdoor surface, bathroom shelf, etc.
- The environment should make the product feel NATURAL and give additional context for its size
- Keep the scene clean and uncluttered — the product is still the focus, but the setting adds warmth and context

🔴 LIGHTING (LIFESTYLE):
- Natural, warm lighting that matches the real-world environment
- Soft ambient light with gentle directional light — realistic home/office/outdoor lighting
- True-to-life color accuracy — the product colors must be accurate
- Both the product and the reference object must be well-lit and clearly visible
`}
...
router.post("/generate", async (req, res) => {
  ...
  try {
    ...
    if (isSizeDimensionMode) {
      ...
      logger.log("📐 [BACKEND] addTextOverlay:", addTextOverlay);
      logger.log("📐 [BACKEND] sceneType:", sceneType);
    ...
    const settingsWithSession = {
      ...
      ...(isSizeDimensionMode && { isSizeDimensionMode: true }),
    };
    ...
    if (isSizeDimensionMode || isDetailCloseupMode || isLifestyleMode || isInfographicMode || isColorChange || isPoseChange || isRefinerMode) {
      // 📐 SIZE/DIMENSION MODE, 🔍 DETAIL CLOSEUP MODE, 🌿 LIFESTYLE MODE, 📊 INFOGRAPHIC MODE, 🎨 COLOR CHANGE MODE, 🕺 POSE CHANGE MODE veya 🔧 REFINER MODE
      if (isSizeDimensionMode) {
        logger.log(
        ...
        enhancedPrompt = await enhancePromptWithGemini(
          ...
          sceneType // "lifestyle" veya "studio"
        );
      } else if (isDetailCloseupMode) {
        logger.log(
