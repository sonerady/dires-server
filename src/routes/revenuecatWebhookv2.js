...
router.post("/webhookv2", async (req, res) => {
  try {
    ...
    // grant trial_credits (default 100) instead of the full package credits.
    ...
    if (period_type === "TRIAL" && type === "INITIAL_PURCHASE") {
      ...
      // veriyoruz. Trial başlangıcında verilen ~100 trial bonusunu DÜŞMÜYORUZ,
      // bonus olarak kalsın (trial'ı tamamlayan kullanıcıya teşekkür jesti).
      console.log(
        `🎉 [RC_WEBHOOK_V2] Trial-to-Paid CONVERSION detected (is_trial_conversion=true) → granting full ${packageCredits} credits (trial 100 bonus preserved on top)`,
      );
    ...
    if (isTrialGrant) {
      isPro = false;
      console.log(
        `🚫 [RC_WEBHOOK_V2] Trial grant → forcing is_pro=false (watermark stays on)`,
      );
    }
    ...
    if (planType) {
      ...
      // 1 trial = 1 team member = 2 kullanıcı 100 krediyi paylaşır → suistimal riski.
