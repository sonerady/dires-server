...
async function processBulkColorItem({
  ...
  try {
    ...
    // her denemede yeniden kurulur: falModel GPT → NB yedeğine düşünce quality alanı değişir
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (falModel === GPT25_EDIT_MODEL && attempt < maxRetries) {
          ...
          console.warn(
            `🛟 [BULK_COLOR] Item ${index}: GPT Image 2.5 başarısız (status=${err.response?.status}, ${err.message}); nano-banana-2'ye geçiliyor`
          );
