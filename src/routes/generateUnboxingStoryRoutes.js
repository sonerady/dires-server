...
router.post("/generate-unboxing-story", async (req, res) => {
    ...
    try {
        ...
        let brandLine = '';
        const hasUserBrandAssets = !!(brandName || brandLogoUrl || customPackageUrl);
        ...
        // Marka bilgisi YOKSA: AI'a kutuya hiçbir logo/marka/yazı eklememesini açıkça söyle
        if (!hasUserBrandAssets) brandLine += `\nCRITICAL — NO BRAND MARKINGS: The user has NOT provided any brand assets. The package box and packaging in ALL scenes MUST be COMPLETELY PLAIN and UNBRANDED. Do NOT add any logos, brand names, text, printed labels, monograms, slogans, watermarks, or graphic markings on the box, ribbon, tissue paper, or any packaging. The box should be a clean unmarked premium box — no visible branding of ANY kind. Do NOT invent or add fictional brand names or logos.\n`;

        // Scene descriptions: user custom or defaults
        // NOT: Marka detayları (logo / brand text) yalnızca kullanıcı brand asset'i yüklediyse
        // brandLine üzerinden dahil edilir. Default scene1 metni marka-nötr tutulur.
        const scene1 = up.scene_1_instruction || `Hands holding a premium garment-sized package box at home — a sturdy box made of thick matte cardstock with crisp clean edges, satin or grosgrain ribbon, optional magnet-closure flap. ${hasUserBrandAssets ? '' : 'The box exterior is COMPLETELY PLAIN and UNBRANDED — no logos, no printed text, no monograms, no labels, no graphic markings of any kind. '}Style and color of the box should harmonize with the product — refined luxury packaging quality. The box must look BRAND-NEW: no scuff marks, no creases, no worn corners, no plain shipping cardboard texture. NOT a generic brown shipping carton, NOT a flimsy mailer bag, NOT an oversized warehouse box. Excited unboxing moment, cozy indoor setting on couch or bed.`;
...
{"scene_1":"transform ...","scene_2":"transform ...","scene_3":"transform ...","scene_4":"transform ...","scene_5":"transform ...","scene_6":"transform ..."}`;
        ...
        // Marka asset'i yoksa, fallback prompt'larında kutunun PLAIN olduğunu açıkça belirt
        const noBrandClause = hasUserBrandAssets
            ? ''
            : ', the box exterior is COMPLETELY PLAIN and UNBRANDED with NO logos NO printed text NO labels NO monograms NO graphic markings of any kind';

        const scenePrompts = [
            prompts.packageArrived || `convert to hands holding a premium garment-sized package box at home on couch or bed — sturdy thick matte cardstock construction with crisp clean edges, satin or grosgrain ribbon, optional magnet-closure flap, refined luxury packaging quality. The box must look BRAND-NEW: no scuff marks, no creases, no worn corners, no plain shipping cardboard texture. NOT a tiny jewelry box, NOT a generic brown shipping carton, NOT a flimsy mailer bag, NOT an oversized warehouse box${brandPackageNote}${noBrandClause}, the box should harmonize in tone with the garment and fit a folded garment inside, excited moment, cozy home interior background, warm indoor ambient lighting, authentic phone camera quality, raw photo only NO social media UI NO buttons NO icons NO app interface, preserve all garment details exactly`,
            prompts.unboxing || `convert to flat lay overhead shot of opened clothing delivery box on bed${brandPackageNote}${noBrandClause}, the garment neatly folded inside with tissue paper wrapping, a cute handwritten-style white text overlay on the photo saying siparisim geldi cok tesekkurler with a heart emoji, phone camera from above, warm indoor lighting, authentic unboxing moment, raw photo only NO social media UI NO buttons NO icons NO app interface, preserve all garment details exactly`,
...
router.post("/retry-unboxing-scene", async (req, res) => {
    ...
    try {
        ...
        if (userId && userId !== "anonymous_user") {
            ...
            retryHasBrandAssets = !!(brandPrefs?.brand_name || brandPrefs?.brand_logo_url || brandPrefs?.custom_package_url);
        }
        const retryNoBrandClause = retryHasBrandAssets
            ? ''
            : ', the box exterior is COMPLETELY PLAIN and UNBRANDED with NO logos NO printed text NO labels NO monograms NO graphic markings of any kind';

        // Fallback prompts
        const defaultPrompts = [
            `convert to hands holding a premium garment-sized package box at home on couch or bed — sturdy thick matte cardstock construction with crisp clean edges, satin or grosgrain ribbon, optional magnet-closure flap, refined luxury packaging quality. The box must look BRAND-NEW: no scuff marks, no creases, no worn corners, no plain shipping cardboard texture. NOT a tiny jewelry box, NOT a generic brown shipping carton, NOT a flimsy mailer bag, NOT an oversized warehouse box${retryBrandNote}${retryNoBrandClause}, the box should harmonize in tone with the garment and fit a folded garment inside, excited moment, cozy home interior background, warm indoor ambient lighting, authentic phone camera quality, raw photo only NO social media UI NO buttons NO icons NO app interface, preserve all garment details exactly`,
            `convert to flat lay overhead shot of opened clothing delivery box on bed${retryBrandNote}${retryNoBrandClause}, the garment neatly folded inside with tissue paper wrapping, a cute handwritten-style white text overlay on the photo saying siparisim geldi cok tesekkurler with a heart emoji, phone camera from above, warm indoor lighting, authentic unboxing moment, raw photo only NO social media UI NO buttons NO icons NO app interface, preserve all garment details exactly`,
