...
async function callReplicateNanoBananaPro(prompt, resultImageUrl, referenceImageUrl, maxRetries = 3, imageSize = "9:16") {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            console.log(`🍌 [UNBOXING_BANANA2] Image generation attempt ${attempt}/${maxRetries}`);
            console.log(`🍌 [UNBOXING_BANANA2] Prompt: ${prompt.substring(0, 100)}...`);
            ...
            console.log(`⏳ [UNBOXING_BANANA2] Prediction created, id: ${prediction.id}`);
            ...
            for (let poll = 0; poll < maxPolls; poll++) {
                ...
                console.log(`⏳ [UNBOXING_BANANA2] Poll ${poll + 1}/${maxPolls}, status: ${result.status}`);
                ...
                if (result.status === "succeeded") {
                    ...
                    if (output) {
                        ...
                        if (imageUrl) {
                            console.log(`✅ [UNBOXING_BANANA2] Image generated successfully`);
                            return imageUrl;
                        }
                    }
            ...
            console.error(`❌ [UNBOXING_BANANA2] Attempt ${attempt} failed:`, error.message);
