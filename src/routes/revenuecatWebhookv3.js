...
router.post("/webhookv3", async (req, res) => {
  ...
  res.on("finish", () => {
    ...
    syncOneSignalTagsFromDb(uid).catch((err) =>
      console.warn("[OS-SYNC v3] error:", err?.message || err),
    );
  });
  ...
  try {
    ...
    if (expectedWebhookAuth) {
      if (req.headers.authorization !== expectedWebhookAuth) {
        console.warn(
          "🚫 [RC_WEBHOOK_V3] Geçersiz/eksik webhook auth header — istek reddedildi (401).",
        );
    ...
    // grant trial_credits (default 2000) instead of full package credits.
    ...
    if (period_type === "TRIAL" && type === "INITIAL_PURCHASE") {
      ...
      console.log(
        `🎉 [RC_WEBHOOK_V3] Trial-to-Paid CONVERSION detected (is_trial_conversion=true) → granting full ${packageCredits} credits (trial 2000 bonus preserved on top)`,
      );
