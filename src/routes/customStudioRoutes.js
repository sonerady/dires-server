...
async function processTool(row) {
 ...
 try {
  ...
  if(!plan){
   ...
   plan=brief.examples;
   check(await db.from(TABLE).update({title:brief.title,brief:brief.brief,screen_schema:brief.screen,translations:{[row.language]:{title:brief.title,description:brief.brief}},example_plan:plan,updated_at:new Date().toISOString()}).eq('id',row.id).is('deleted_at',null));
  }
  const accepted=[...(row.accepted_examples||[])];
  for(let i=0;i<4;i++){
   if(!check(await db.from(TABLE).select('id').eq('id',row.id).is('deleted_at',null).maybeSingle()))return;
   if(accepted.some(e=>e.index===i))continue;
   let feedback='',sample;
   ...
   const changes={accepted_examples:accepted,updated_at:new Date().toISOString()};
   if(i===0){changes.before_url=sample.beforeUrl;changes.after_url=sample.afterUrl;}
   check(await db.from(TABLE).update(changes).eq('id',row.id).is('deleted_at',null));
  }
  check(await db.from(TABLE).update({status:'completed',intro_examples:accepted.filter(e=>e.index>0).sort((a,b)=>a.index-b.index),error_code:null,updated_at:new Date().toISOString()}).eq('id',row.id).is('deleted_at',null).eq('status','processing'));
 } catch(error) {
...
async function tick() {
 ...
 try {
  ...
  const rows=check(await db.from(TABLE).select('*').eq('status','queued').order('created_at').limit(2));
  await Promise.all(rows.map(async row=>{
   ...
   if(claimed)await processTool(claimed);
  }));
...
router.post('/review',rateLimit({windowMs:60000,limit:6,standardHeaders:true,legacyHeaders:false}),upload.fields([{name:'photos',maxCount:6}]),wrap(async(req,res)=>{
 ...
 let input,previous=null,reference_urls=[],feedback='';
 try{
  if(req.body.reviewToken){
   ...
   reference_urls=previous.reference_urls;
 ...
 if(!previous){
  try{reference_urls=await Promise.all((req.files?.photos||[]).map((f,i)=>saveImage(f.buffer,owner,randomUUID(),`idea-reference-${i}`)));}
  catch{return res.status(400).json({success:false,reason:'invalid_photo'});}
 }
 const review=await reviewTool({description:input.description,language:input.language,images:reference_urls,previous:previous?.review||null,feedback});
 const reviewToken=issueReview({owner,requestKey:input.request_key,language:input.language,originalDescription:input.description,reference_urls,review});
...
router.post('/',upload.fields([{name:'photos',maxCount:6},{name:'photo',maxCount:1}]),wrap(async(req,res)=>{
 ...
 const result=await db.from(TABLE).insert({...input,id,user_id:owner,reference_urls,button_hue:toolHue({id,description:input.description})}).select('*').single();
