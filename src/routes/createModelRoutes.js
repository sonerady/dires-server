...
async function transformImageToIDPhoto(imageUrl, userId, hijabPrompt = null, promptOverride = null) {
  try {
    logger.log(
    ...
    // promptOverride: model havuzu (POOL_PORTRAIT_PROMPT) gibi kimliği KORUMAYAN varyantlar.
    let transformPrompt = promptOverride || REFERENCE_ID_PHOTO_PROMPT;
...
router.use(require("./starterModelRoutes")({
  ...
  createPortrait: async ({ imageUri, languageCode = "en", regionCode = "US" }) => {
    ...
    // Kimlik korunmaz: saç + yüz hatlarında hafif farklarla YENİ kurgusal model (telif).
    const portrait = await transformImageToIDPhoto(uploadedImageUrl, owner, null, POOL_PORTRAIT_PROMPT);
    return {
      name: analysis?.suggestedName || "Model", gender, age: analysis?.detectedAge != null ? String(analysis.detectedAge) : "young",
      // Boy/beden girilmez: profil boş gelir, kullanıcı isterse sonra doldurur.
      imageUrl: portrait.imageUrl, originalImageUrl: uploadedImageUrl, modelProfile: {},
    };
  },
  // Yeniden işleme (mevcut havuz kayıtları): orijinal fotoğraftan yeni varyant portre.
  regeneratePortrait: async ({ imageUrl }) => (await transformImageToIDPhoto(imageUrl, "model-pool", null, POOL_PORTRAIT_PROMPT)).imageUrl,
  // 70 dilde modern, o dile ait isimler — DeepSeek (havuzdaki isimler dışlanır).
