  ...
  try {
    ...
    try {
      ...
      if (isTeamCredit && effectiveCredits.creditOwnerId) {
        ...
        console.log(`👥 [VIDEO-V2] Team member detected - using owner credits`);
        console.log(`   Member: ${userId}`);
        console.log(`   Owner (credit source): ${effectiveUserId}`);
        console.log(`   Available credits: ${effectiveCreditBalance}`);
      }
    } catch (teamError) {
      console.log(`⚠️ [VIDEO-V2] Team check failed, using user's own credits:`, teamError.message);
    ...
    if (isTeamCredit) {
      console.log(`✅ [VIDEO-V2] Credits deducted from team owner: ${effectiveUserId}`);
