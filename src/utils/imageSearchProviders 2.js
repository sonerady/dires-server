...
//   APIFY_MAX_CONCURRENCY  (varsayılan: 10 — fetch_cat paralelliği)
// ─────────────────────────────────────────────────────────────
...
function buildApifyInput(actor, query, limit) {
  ...
  //
  // maxConcurrency: ölçümde saf kazıma süresi 4,5-40 sn arasında salınıyordu.
  // Varsayılan eşzamanlılık düşük olduğu için yükseltmek işi paralelleştirebilir.
  // runBudgetSeconds: aktörün kendi tavanı — bizim HTTP tavanımızdan biraz kısa
  // tutuluyor ki iş yarıda kalıp para yanmasın.
  const concurrency = parseInt(process.env.APIFY_MAX_CONCURRENCY, 10) || 10;
  const budget = Math.max(
    Math.round(((parseInt(process.env.APIFY_TIMEOUT_MS, 10) || 60000) - 5000) / 1000),
    15,
  );
  return {
    queries: [query],
...
async function searchApify(query, limit) {
  ...
  try {
    ...
    err.message = `${err.message} [${secs} sn sonra, limit=${limit}, tavan=${Math.round(timeout / 1000)} sn]`;
    throw err;
  }
