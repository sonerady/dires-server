create table public.credit_refund_charges (
 user_id uuid not null references public.users(id) on delete cascade,
 generation_id text not null,
 result_id uuid references public.reference_results(id) on delete set null,
 credit_owner_id uuid not null references public.users(id),
 credits integer not null check(credits>0),
 result_image_url text not null,
 product_image_urls jsonb not null,
 created_at timestamptz not null default now(),
 primary key(user_id,generation_id)
);
alter table public.credit_refund_charges enable row level security;
revoke all on public.credit_refund_charges from public,anon,authenticated;
grant all on public.credit_refund_charges to service_role;

-- Ignore caller-supplied ownership: automatic settlement requires the private charge proof.
